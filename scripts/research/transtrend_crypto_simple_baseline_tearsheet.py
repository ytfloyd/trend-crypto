...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Transtrend Crypto simple baseline tearsheet")
...
def main() -> None:
    ...
    metrics_path = Path(args.metrics_csv) if args.metrics_csv else research_dir / "metrics_transtrend_crypto_simple_baseline.csv"
    if not metrics_path.exists():
        print(f"[transtrend_crypto_simple_baseline_tearsheet] Metrics CSV not found: {metrics_path}; computing stats from equity.")
    ...
    build_standard_tearsheet(
        ...
        strategy_label="Transtrend Crypto Simple Baseline (Fixed Universe, Spot Long-Only)",
        ...
        subtitle="MA(20/100) equal-weight longs; fixed universe; no risk overlays",
    )
