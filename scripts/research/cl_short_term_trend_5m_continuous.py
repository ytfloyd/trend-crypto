...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="5m long/short short-term trend on CL continuous futures.")
    ...
    parser.add_argument("--out_dir", default="artifacts/research/cl_short_term_trend_5m_continuous")
...
def build_strategy(bars: pd.DataFrame, lookback: int, entry_z: float, exit_z: float, cost_bps: float) -> pd.DataFrame:
    ...
    # Signal at close t, fill next open, earn open-to-next-open futures return.
    df["zscore"] = zscore
    df["w_signal"] = signal
...
def write_figures(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    axes[0].set_title("CL Continuous 5m Short-Term Trend")
...
def main() -> None:
    ...
    config = {
        ...
        "rule": "trend: long z>=entry_z, short z<=-entry_z, exit on z crossing 0",
    ...
    print(f"[cl_short_term_trend_5m_continuous] wrote artifacts to {out_dir}")
