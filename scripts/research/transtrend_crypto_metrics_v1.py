...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compute metrics for Transtrend Crypto v1")
    p.add_argument("--equity", required=True, help="Path to equity.csv")
    p.add_argument("--out", required=True, help="Output metrics CSV path")
    p.add_argument("--stop_events", default=None, help="Optional stop_events.csv path")
    return p.parse_args()
...
def compute_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    avg_turnover_two = df["turnover_two_sided"].mean() if "turnover_two_sided" in df.columns else 0.0
    ...
    out = {
        ...
        "avg_turnover_two_sided": avg_turnover_two,
    }
...
def main() -> None:
    ...
    stop_events_path = Path(args.stop_events) if args.stop_events else Path(args.equity).with_name("stop_events.csv")
    if stop_events_path.exists():
        stop_events = pd.read_csv(stop_events_path)
        stop_hits = len(stop_events)
        n_days = metrics.loc[0, "n_days"] if not metrics.empty else 0
        rate = stop_hits / (n_days / 365.0) if n_days > 0 else 0.0
        metrics.loc[0, "stop_hits"] = stop_hits
        metrics.loc[0, "stop_hit_rate_per_year"] = rate

    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    metrics.to_csv(out_path, index=False)
    print(f"[transtrend_crypto_metrics_v1] Wrote metrics to {out_path}")
