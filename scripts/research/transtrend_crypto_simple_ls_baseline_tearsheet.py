...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Transtrend Crypto simple L/S baseline tearsheet")
...
def main() -> None:
    ...
    metrics_path = (
        ...
        if args.metrics_csv
        else research_dir / "metrics_transtrend_crypto_simple_ls_baseline.csv"
    )
    if not metrics_path.exists():
        print(
            f"[transtrend_crypto_simple_ls_baseline_tearsheet] Metrics CSV not found: {metrics_path}; computing stats from equity."
        )
    ...
    build_standard_tearsheet(
        ...
        strategy_label="Transtrend Crypto Simple L/S Baseline (Fixed Universe)",
        ...
        subtitle="MA(20/100) sign; equal-weight long/short; fixed universe; no overlays",
    )
