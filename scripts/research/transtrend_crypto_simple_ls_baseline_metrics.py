...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Metrics for Transtrend Crypto simple L/S baseline")
...
def compute_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    avg_net = df["net_exposure"].mean() if "net_exposure" in df.columns else 0.0
...
def main() -> None:
    ...
    print(f"[transtrend_crypto_simple_ls_baseline_metrics] Wrote metrics to {out_path}")
