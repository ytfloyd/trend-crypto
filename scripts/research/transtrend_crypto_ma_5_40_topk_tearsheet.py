...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Transtrend Crypto MA(5/40) Top-K tearsheet")
...
def main() -> None:
    ...
    metrics_path = (
        ...
        if args.metrics_csv
        else research_dir / "metrics_transtrend_crypto_ma_5_40_topk.csv"
    )
    if not metrics_path.exists():
        print(
            f"[transtrend_crypto_ma_5_40_topk_tearsheet] Metrics CSV not found: {metrics_path}; computing stats from equity."
        )
    ...
    build_standard_tearsheet(
        ...
        strategy_label="Transtrend Crypto MA(5/40) Top-K (Long-Only)",
        ...
        subtitle="MA(5/40) long-only; top-K ranking; dynamic universe",
    )
