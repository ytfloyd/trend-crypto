...
trailing dollar-ADV (survivorship-free, no look-ahead):
  ...
  membership   : the COMMITTED bars_1d..top50_adv10m_membership table (validated baseline)

RECONCILED METHOD (v2). The composite is a *cross-sectional rank*, so it MUST be
ranked WITHIN each candidate universe — otherwise a name's percentile depends on
the full 362-name population, conflating "universe" with "ranking population".
We compute per-symbol factors once (their rolling inputs are population-independent),
then for each universe mask the factor frames to the eligible set BEFORE ranking.
The `membership` spec reproduces the validated walk-forward baseline (~2.03) so the
ADV-floor / top_N numbers are apples-to-apples with it.

Params FROZEN at flagship defaults (entry 0.65 / trail 0.15) for the sweep; the
walk-forward block fold-selects from the standard grid. 30 bps one-way. OOS = 2023+.
...
import run_medallion_walkforward as wf  # noqa: E402  (reuse _daily / vol_target / GRID / FOLDS / _sortino)
...
def load_universe_panel():
    """Load hourly panel + per-symbol FACTORS + regime ONCE for the full USD universe,
    plus a daily trailing-ADV table used to build every point-in-time membership mask.
    Returns factors (dict of wide frames), rw, regime, dd (daily adv), and the committed
    membership long-frame for the reconciliation baseline."""
    ...
    memb = con.execute(f"SELECT ts, symbol FROM {MEMBERSHIP} WHERE ts>=? AND ts<=?",
                       [START, END]).fetch_df()
    ph = ", ".join(["?"] * len(syms))
    hf = con.execute(
    ...
    print("  computing regime + per-symbol factors (once) ...")
    regime = compute_ensemble_regime(btc_h.resample("D").last().dropna(), btc_h, rw)
    factors = compute_factors(cw, vw, hw)
    return factors, rw, regime, dd, memb


def eligibility(dd, memb, kind, value, columns, hourly_index) -> pd.DataFrame:
    """Point-in-time hourly eligibility mask for one universe spec."""
    if kind == "membership":
...
def composite_within(factors: dict, elig_h: pd.DataFrame) -> pd.DataFrame:
    """Rank factors WITHIN the eligible universe (mask before cross-sectional rank),
    then keep only eligible cells. This is the reconciliation fix."""
    masked = {k: v.where(elig_h) for k, v in factors.items()}
    return compute_composite_score(masked).where(elig_h)


def _config_daily(comp_pit, rw, regime, prm) -> pd.Series:
    ...
    turn = float(np.mean(bt["turnover"])) if "turnover" in bt else float("nan")
    return daily, turn


def run_spec(factors, rw, regime, elig_h):
    comp_pit = composite_within(factors, elig_h)
    daily, turn = _config_daily(comp_pit, rw, regime, PARAMS)
    return daily, float(elig_h.sum(axis=1).mean()), turn


def walk_forward_oos(factors, rw, regime, elig_h) -> pd.Series:
    comp_pit = composite_within(factors, elig_h)
    runs = {(p["entry_threshold"], p["trailing_stop_pct"]): _config_daily(comp_pit, rw, regime, p)[0]
            for p in wf.GRID}
    segs = []
...
def build_spec_daily(kind: str, value: float, params: dict | None = None):
    """Reusable: load the panel and return (daily, btc_daily, avg_names, turn) for one
    universe spec with within-universe ranking. Used by the PDF / tearsheet generators."""
    params = params or PARAMS
    factors, rw, regime, dd, memb = load_universe_panel()
    ...
    return daily, btc, float(elig_h.sum(axis=1).mean()), turn
...
def main() -> None:
    ...
    for label, kind, value in SPECS:
        elig_h = eligibility(dd, memb, kind, value, cols, idx)
        daily, avg_names, turn = run_spec(factors, rw, regime, elig_h)
        f, o = _metrics(daily), _metrics(daily, OOS_START)
        ovt = _metrics(wf.vol_target(daily), OOS_START)
        rows.append((label, avg_names, turn, f, o, ovt))
        print(f"{label:<20}{avg_names:>9.0f}{turn:>9.2f} | "
    ...
    print("-" * 110)
    print(f"{'BTC buy&hold':<20}{'':>18} | {'':>28} | "
          f"{bo['sortino']:>9.2f}{bo['sharpe']:>6.2f}{bo['cagr']:>6.0%}{bo['max_dd']:>6.0%}")
    ...
    for label, kind, value in [("membership", "membership", 0), ("top_50", "top", 50),
        ...
        print(f"  {label:<14} WF-OOS Sortino {m['sortino']:.2f}  Sharpe {m['sharpe']:.2f}  "
              f"CAGR {m['cagr']:.0%}  DD {m['max_dd']:.0%}   (+vol-target {mvt['sortino']:.2f})")
    print("\nReconcile: 'membership' here should track the validated 2.03 walk-forward baseline. "
          "All numbers use WITHIN-universe cross-sectional ranking. 30bps, point-in-time ADV.")
