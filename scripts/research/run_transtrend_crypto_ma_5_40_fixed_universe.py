...
def main() -> None:
    ...
    if args.write_bundle:
        ...
        print(f"[transtrend_crypto_ma_5_40_fixed_universe] Wrote timeseries bundle: {bundle_path} (rows={bundle_info.get('rows')})")
    ...
    if not args.no_html:
        ...
        build_standard_html_tearsheet(
            ...
            strategy_label="MA(5/40) Fixed Universe Baseline",
            strategy_equity=strat_eq,
            equity_csv_path=str(out_dir / "equity.csv"),
            manifest_path=str(out_dir / "run_manifest.json"),
            subtitle="Dual moving average (5/40) crossover on fixed cryptocurrency universe",
        )
