#!/usr/bin/env python
"""Transtrend Crypto v0 tear sheet (thin wrapper over canonical builder)."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build Transtrend Crypto v0 tear sheet.")
...
def main() -> None:
    ...
    metrics_path = Path(args.metrics_csv) if args.metrics_csv else research_dir / "metrics_transtrend_crypto_v0.csv"
    if not metrics_path.exists():
        print(f"[transtrend_crypto_tearsheet_v0] Metrics CSV not found: {metrics_path}; computing stats from equity.")
    ...
    build_standard_tearsheet(
        ...
        subtitle=(
            "Multi-horizon breakouts + MA filters; inverse-vol sizing; danger gating; vol target=20%"
        ),
    )
