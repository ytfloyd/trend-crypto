...
def main() -> None:
    ...
    if args.write_bundle:
        ...
        features_df = panel[["ts", "symbol", "score", "vol_ann", "atr"]].copy()
        ...
        print(f"[transtrend_crypto_v1] Wrote timeseries bundle: {bundle_path} (rows={bundle_info.get('rows')})")
    ...
    if not args.no_html:
        ...
        build_standard_html_tearsheet(
            ...
            strategy_label="Transtrend Crypto v1 (Spot + ATR Stops)",
            strategy_equity=strat_eq,
            equity_csv_path=str(out_dir / "equity.csv"),
            manifest_path=str(out_dir / "run_manifest.json"),
            subtitle="Spot long-only trend-following with ATR trailing stops",
        )
