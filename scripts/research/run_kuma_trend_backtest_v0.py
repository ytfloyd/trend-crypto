...
def main() -> None:
    ...
    if args.write_bundle:
        ...
        weights_signal_df = weights_df.reset_index().rename(columns={"weight": "w_signal"})
        signals_df = positions.reset_index()[["ts", "symbol", "signal"]].copy()
        ...
        bundle_info = write_timeseries_bundle(
            ...
            weights_signal_df=weights_signal_df,
            portfolio_df=equity_df,
            write_parquet=write_parquet,
            write_csvgz=write_csvgz,
        )
        bundle_path = bundle_info.get("parquet") or bundle_info.get("csvgz")
        print(f"[run_kuma_trend_backtest_v0] Wrote timeseries bundle: {bundle_path} (rows={bundle_info.get('rows')})")
    ...
    if bundle_info:
        if bundle_info.get("parquet"):
            artifacts_written["bundle_parquet"] = bundle_info["parquet"]
        if bundle_info.get("csvgz"):
            artifacts_written["bundle_csvgz"] = bundle_info["csvgz"]
    ...
    if not args.no_html:
        ...
        build_standard_html_tearsheet(
            ...
            strategy_label="Kuma Trend (Breakout Trend-Following)",
            strategy_equity=strat_eq,
            equity_csv_path=str(out_dir / "equity.csv"),
            manifest_path=str(out_dir / "run_manifest.json"),
            subtitle="Breakout trend-following with momentum ranking",
        )
