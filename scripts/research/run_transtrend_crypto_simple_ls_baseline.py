...
from transtrend_crypto_simple_ls_baseline_lib import (
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Transtrend Crypto simple L/S baseline (fixed universe)")
    ...
    p.add_argument("--out_dir", type=str, default="artifacts/research/transtrend_crypto_simple_ls_baseline")
...
def main() -> None:
    ...
    if args.write_bundle:
        ...
        print(f"[transtrend_crypto_simple_ls_baseline] Wrote timeseries bundle: {bundle_path} (rows={bundle_info.get('rows')})")
    ...
    manifest = build_base_manifest("transtrend_crypto_simple_ls_baseline", sys.argv)
    manifest.update(
        {
            "config": config_blob,
    ...
    print(f"[transtrend_crypto_simple_ls_baseline] Wrote artifacts to {out_dir}")
    ...
    if not args.no_html:
        ...
        build_standard_html_tearsheet(
            ...
            strategy_label="Simple L/S Baseline (Fixed Universe)",
            strategy_equity=strat_eq,
            equity_csv_path=str(out_dir / "equity.csv"),
            manifest_path=str(out_dir / "run_manifest.json"),
            subtitle="Simple long/short dual MA crossover baseline on fixed cryptocurrency universe",
        )
