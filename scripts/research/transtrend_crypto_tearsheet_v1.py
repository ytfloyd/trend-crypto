#!/usr/bin/env python
"""Transtrend Crypto v1 tear sheet (ATR stop overlay)."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build Transtrend Crypto v1 tear sheet.")
...
def main() -> None:
    ...
    metrics_path = Path(args.metrics_csv) if args.metrics_csv else research_dir / "metrics_transtrend_crypto_v1.csv"
    if not metrics_path.exists():
        print(f"[transtrend_crypto_tearsheet_v1] Metrics CSV not found: {metrics_path}; computing stats from equity.")
    ...
    build_standard_tearsheet(
        ...
        strategy_label="Transtrend Crypto v1 (Spot Long-Only + ATR Stops)",
        ...
        subtitle=(
            "ATR(20) trailing stops (k=3.0, cooldown=5d); inverse-vol sizing; danger gating; vol target=20%"
        ),
    )
