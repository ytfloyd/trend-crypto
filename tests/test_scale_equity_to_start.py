...
def test_scale_equity_to_start_basic():
    ...
    assert scaled.iloc[0] == pytest.approx(100000.0)
    assert scaled.iloc[1] == pytest.approx(110000.0)
    assert scaled.iloc[2] == pytest.approx(105000.0)
    assert scaled.iloc[3] == pytest.approx(120000.0)
...
def test_scale_equity_to_start_preserves_shape():
    ...
    assert scaled.iloc[0] == pytest.approx(50.0)
    assert scaled.iloc[1] == pytest.approx(100.0)
    assert scaled.iloc[2] == pytest.approx(25.0)
