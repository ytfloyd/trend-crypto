...
def test_resolve_ambiguous_equity(tmp_path: Path):
    eq1 = tmp_path / "ensemble_equity_v0.csv"
    eq2 = tmp_path / "ensemble_equity_alt.csv"
    met = tmp_path / "metrics_101_ensemble_filtered_v1.csv"
    eq1.write_text("ts,equity\n2020-01-01,1.0\n")
    eq2.write_text("ts,equity\n2020-01-01,1.0\n")
    met.write_text("period,cagr\nfull,0.1\n")
    try:
        resolve_tearsheet_inputs(research_dir=str(tmp_path))
        assert False, "Expected ValueError due to multiple equity candidates"
    except ValueError as e:
        msg = str(e)
        assert "equity" in msg.lower()
        assert "ensemble_equity" in msg
