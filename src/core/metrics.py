"""
Canonical performance metrics and formatting utilities (pandas/equity-curve flavor).

Single source of truth for the equity-curve metrics used across research. Moved
here verbatim from ``scripts/research/common/metrics.py`` (which now re-exports
from this module) as the first step of the core-stack consolidation; numbers are
unchanged. See docs/RESEARCH_PIPELINE_REORGANIZATION.md.

Note: this is the pandas/``ANN_FACTOR``-based flavor used by research scripts. The
polars/engine flavor (``src/common/metrics.py``, ``src/backtest/metrics.py``)
differs subtly (e.g. CAGR annualization exponent over n vs n-1 periods, and
degenerate-vol sentinels of NaN vs 0.0). Reconciling the two flavors into one set
of primitives is a tracked follow-up, not part of this verbatim move.
"""
...
# Crypto trades every day; annualization factor for daily bars.
# Mirrors ``scripts/research/common/data.ANN_FACTOR`` (kept in sync at 365.0).
ANN_FACTOR = 365.0
...
def format_metrics_table(
    results: dict[str, Any] | list[dict[str, Any]], label_key: str = "label"
) -> str:
